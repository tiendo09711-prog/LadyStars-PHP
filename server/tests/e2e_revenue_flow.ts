async function runTest() {
  ...
  try {
    ...
    if (!loginRes.ok) throw new Error(`Đăng nhập thất bại: ${loginRes.statusText}`);
    ...
    const todayData1 = revenueData1.find((item: any) => item.time === todayStr) || { revenue: 0 };
    ...
    if (!invoiceRes.ok) throw new Error(`Tạo hóa đơn thất bại: ${await invoiceRes.text()}`);
    ...
    const todayData2 = revenueData2.find((item: any) => item.time === todayStr) || { revenue: 0 };
