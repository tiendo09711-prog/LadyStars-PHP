...
function matchesTab(invoice: Invoice, tab: WholesaleTab): boolean {
  ...
  if (tab === 'debt') {
    ...
    return netValue(invoice) - Number(invoice.valuePayment || 0) > 0;
  }
  return true;
}
...
export function WholesaleInvoicePage({ channel }: WholesaleInvoicePageProps) {
  ...
  const buildPrintDocument = (invoice: Invoice, branch: Branch | null, shop: any, items: any[], title: string, hideTotals = false) => {
    ...
    const totalAmount = netValue(invoice);
    ...
    return buildReceiptHtml({
      ...
      summary: hideTotals ? [] : [
        ...
        ...(invoice.hasVat && Number(invoice.vatAmount) > 0
          ...
          : []),
        { label: 'Thành tiền', value: safeMoney(netValue(invoice)), strong: true },
  ...
  const handleDeleteInvoice = async (invoice: Invoice) => {
    ...
    const confirmation = window.confirm(
      [
        ...
        `Tổng tiền: ${safeMoney(netValue(invoice))}`,
                ...
                ? pagedInvoices.map((invoice) => {
                    ...
                    return (
                      <tr key={invoice._id}>
                        ...
                        <td className="number total" title={safeMoney(netValue(invoice))}>
                          {safeMoney(netValue(invoice))}
                        </td>
      ...
      <aside className="ws-detail-side">
        <section className="ws-detail-card">
          ...
          <dl className="ws-money-summary">
            ...
            <div className="grand"><dt>Tổng tiền</dt><dd>{safeMoney(netValue(invoice))}</dd></div>
