...
export function WarehouseTransferCreatePage() {
  ...
  const searchHelper = warehousesValid ? `Chỉ hiển thị sản phẩm còn tồn tại ${sourceName}.` : 'Chọn kho nguồn và kho đích trước khi tìm sản phẩm.';
  ...
  useEffect(() => {
    ...
    http.get(`/warehouse/transfers/${id}`).then((response) => {
      ...
      if (data.status !== 'DRAFT') setError('Chỉ được sửa đơn ở trạng thái Chờ xác nhận xuất.');
