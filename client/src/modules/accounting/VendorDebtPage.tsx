...
export function VendorDebtPage() {
  ...
  useEffect(() => {
    http.get('/accounting/debt/vendors/stats').then(res => setStats(res.data)).catch(() => {});
  }, []);

  useEffect(() => {
    setLoading(true);
    http.get(`/accounting/debt/vendors/summary?tab=${activeTab}&page=${page}&limit=50`)
      .then(res => {
  ...
  const handleExportCSV = () => {
    const csv = [
      ['Nhà cung cấp', 'SĐT', 'Nợ đầu kì', 'Có đầu kì', 'Ghi nợ', 'Ghi có', 'Nợ cuối kì', 'Có cuối kì'].join(','),
      ...items.map(item => [
    ...
    link.href = url;
    link.download = `cong_no_ncc_${Date.now()}.csv`;
    link.click();
  ...
  return (
    <div className="debt-page">
      ...
      <div className="debt-filters">
        ...
        <input className="debt-filter-input" placeholder="Nhà cung cấp" style={{ width: 200 }} />
        <CustomSelect 
          options={vendorGroupOptions}
          value={vendorGroup}
          onChange={setVendorGroup}
      ...
      <div className="debt-actions-bar">
        <div className="debt-actions-left">
          ...
          <button className="btn-outline-debt">
            <Plus size={14} /> Tính tổng phải trả nhà cung cấp
          </button>
        </div>
        ...
        <table className="debt-table">
          <thead>
            <tr>
              ...
              <th rowSpan={2} className="text-left">Nhà cung cấp<br/>[2]</th>
          ...
          <tbody>
            ...
            {loading ? (
              <tr><td colSpan={10} className="text-center" style={{padding: 40}}>Đang tải dữ liệu...</td></tr>
            ) : items.map((item, index) => (
              <tr key={item._id}>
                <td className="text-center">{(page - 1) * 50 + index + 1}</td>
                <td className="text-left text-blue">{item.code} - {item.vendorName}</td>
            ...
            {items.length === 0 && !loading && (
              <tr><td colSpan={10} className="text-center" style={{padding: 40}}>Chưa có dữ liệu phù hợp</td></tr>
            )}
          </tbody>
        </table>
      </div>
