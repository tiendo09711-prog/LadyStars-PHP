...
export function BankReceiptCreatePage() {
  ...
  const [afterSaveAction, setAfterSaveAction] = useState<'continue' | 'list' | 'customer_debt'>('list');
  ...
  const handleSave = async (e: React.FormEvent, print: boolean = false) => {
    ...
    try {
      const payload = {
        ...
        revenue: form.type === 'Báo có (Nộp tiền)' ? form.amount : 0,
        expense: form.type === 'Báo nợ (Chuyển tiền)' ? form.amount : 0,
      ...
      await http.post('/accounting/bank-transactions', payload);
      ...
      } else if (afterSaveAction === 'customer_debt') {
        navigate('/accounting/debt'); // Or specific customer debt path
      } else {
        setForm((prev) => ({
          ...prev,
          targetName: '',
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '24px', fontFamily: 'Inter, system-ui, sans-serif' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
        <button 
          onClick={() => navigate('/accounting/bank')}
          style={{ 
      ...
      <form style={{ display: 'grid', gridTemplateColumns: '1fr 400px', gap: '24px', alignItems: 'start' }}>
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', padding: '24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              <span style={{ fontSize: '13px', color: '#64748b' }}>Tài khoản ngân hàng <span style={{color: '#ef4444'}}>*</span></span>
              <select 
                value={form.accountName} 
                onChange={(e) => handleChange('accountName', e.target.value)}
                style={{ border: '1px solid #cbd5e1', borderRadius: '6px', padding: '10px 12px', outline: 'none', color: '#1e293b', fontSize: '14px' }}
              >
                <option value="Tiền chuyển khoản HN">Tiền chuyển khoản HN</option>
                <option value="Tiền chuyển khoản HCM">Tiền chuyển khoản HCM</option>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              <select 
                ...
                style={{ border: '1px solid #cbd5e1', borderRadius: '6px', padding: '10px 12px', outline: 'none', color: '#1e293b', fontSize: '14px' }}
              >
                <option value="Báo có (Nộp tiền)">Báo có (Nộp tiền)</option>
                <option value="Báo nợ (Chuyển tiền)">Báo nợ (Chuyển tiền)</option>
              </select>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
              ...
              <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '14px', color: '#475569' }}>
                <input 
                  type="radio" 
                  name="afterSave" 
                  checked={afterSaveAction === 'customer_debt'} 
                  onChange={() => setAfterSaveAction('customer_debt')} 
                  style={{ cursor: 'pointer' }}
                />
                Về trang công nợ khách hàng
              </label>
            </div>
