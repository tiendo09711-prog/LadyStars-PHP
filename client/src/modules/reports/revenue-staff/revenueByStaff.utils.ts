...
export function defaultFilters(): StaffReportFilters {
...
export function filtersToQuery(filters: StaffReportFilters): Record<string, string | number | string[]> {
  ...
  if (filters.staffIds.length > 0) q.staffIds = filters.staffIds;
...
export function filtersToSearchParams(filters: StaffReportFilters, preset: DatePreset): string {
  ...
  if (filters.staffIds.length) p.set('staffIds', filters.staffIds.join(','));
...
export function filtersFromSearchParams(sp: URLSearchParams): {
  ...
  const staffIdsRaw = sp.get('staffIds') || '';
  const staffIds = staffIdsRaw
    ? staffIdsRaw.split(',').map((s) => s.trim()).filter(Boolean)
    : [];
  ...
  return {
    ...
    filters: {
      ...
      sortBy: (sp.get('sortBy') as StaffReportFilters['sortBy']) || 'netRevenue',
...
export function exportRankingExcel(
  ...
  XLSX.writeFile(wb, `bao-cao-doanh-thu-theo-nhan-vien_${filters.from}_${filters.to}.xlsx`);
}
...
export function exportRankingCsv(
  ...
  downloadCsv(`bao-cao-doanh-thu-theo-nhan-vien_${filters.from}_${filters.to}.csv`, headers, rows);
}

export function exportSummaryCsv(report: StaffReportResponse, filters: StaffReportFilters): void {
  const s = report.summary;
  const rows: (string | number)[][] = [
    ['Từ ngày', filters.from],
    ['Đến ngày', filters.to],
    ['Số nhân viên', s.staffCount],
  ...
  if (s.topStaff) {
    rows.push(['Nhân viên dẫn đầu', s.topStaff.name]);
    rows.push(['DT thuần nhân viên dẫn đầu', s.topStaff.netRevenue]);
  }
  ...
  downloadCsv(`bao-cao-doanh-thu-nhan-vien-tong-hop_${filters.from}_${filters.to}.csv`, ['Chỉ số', 'Giá trị'], rows);
}
...
export function metricValue(row: StaffRankingRow, metric: ReportMetric): number {
