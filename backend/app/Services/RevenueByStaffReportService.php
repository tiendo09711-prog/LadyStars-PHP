...
/**
 * Read-only revenue-by-staff report.
 *
 * Business formulas aligned with RevenueByTimeReportService / RevenueByStoreReportService:
 ...
 * - staff key: user_id then author_id for sales; user_id / user_created_id / author_id for refunds
 * - unknown staff group when no staff id can be resolved
 */
class RevenueByStaffReportService
{
    public const TIMEZONE = 'Asia/Ho_Chi_Minh';

    public const UNKNOWN_STAFF_ID = 'unknown';
    ...
    public const TOP_TREND_STAFF = 8;
    ...
    public function options(): array
    {
        ...
        // Include inactive staff so historical filters remain possible (same spirit as inactive stores).
        $staff = User::query()
            ->orderByDesc('is_active')
            ->orderBy('name')
            ->get(['id', 'name', 'email', 'role', 'is_active', 'status'])
            ->map(fn (User $u) => [
                'id' => (string) $u->id,
                'name' => $u->name,
                'email' => $u->email,
                'role' => $u->role,
                'isActive' => $u->is_active === null ? true : (bool) $u->is_active,
                'status' => $u->status,
            ])
            ->values()
            ->all();
        ...
        return [
            ...
            'sortOptions' => [
                ...
                ['value' => 'itemQuantity', 'label' => 'Số sản phẩm'],
                ['value' => 'averageOrderValue', 'label' => 'Giá trị đơn TB'],
                ['value' => 'staffName', 'label' => 'Tên nhân viên'],
            ],
            ...
            'formulas' => [
                ...
                'staffKey' => 'user_id ưu tiên, fallback author_id (sale); user_id / user_created_id / author_id (refund); unknown nếu thiếu',
            ],
        ];
    }
    ...
    public function report(array $input): array
    {
        ...
        $staffMap = $this->loadStaffMap($sales, $refunds);
        $staffRows = $this->aggregateByStaff($sales, $refunds, $staffMap);
        $staffRows = $this->applySearch($staffRows, $filters['search']);

        $summary = $this->buildSummary($staffRows);
        $ranking = $this->buildRanking($staffRows, $filters['metric']);
        $table = $this->paginateRanking($ranking, $filters);
        $trend = $this->buildTrend($filters, $sales, $refunds, $staffMap, $ranking);
        $breakdowns = $this->buildBreakdowns($sales, $refunds, $staffMap, $ranking);
        ...
        if ($filters['compare'] === 'previous_period') {
            ...
            $prevStaffMap = $this->loadStaffMap($prevSales, $prevRefunds);
            $prevRows = $this->aggregateByStaff($prevSales, $prevRefunds, $prevStaffMap);
            $prevRows = $this->applySearch($prevRows, $filters['search']);
            $prevSummary = $this->buildSummary($prevRows);
            $comparison = [
                'period' => [
    ...
    private function normalizeFilters(array $input): array
    {
        ...
        $staffIds = $this->parseStaffIds($input);
        if ($staffIds !== []) {
            $existing = User::query()->whereIn('id', $staffIds)->pluck('id')->map(fn ($id) => (string) $id)->all();
            $missing = array_diff($staffIds, $existing);
            if ($missing !== []) {
                throw ValidationException::withMessages([
                    'staffIds' => ['Một hoặc nhiều nhân viên không tồn tại: '.implode(', ', $missing)],
                ]);
            }
        }
    ...
    private function parseStaffIds(array $input): array
    {
        $raw = $input['staffIds'] ?? $input['staffId'] ?? null;
        if ($raw === null || $raw === '' || $raw === []) {
            return [];
        }
        if (is_array($raw)) {
    ...
    private function loadSales(array $filters): Collection
    {
        ...
        if ($filters['staffIds'] !== []) {
            $ids = array_map('intval', $filters['staffIds']);
            $query->where(function ($q) use ($ids) {
                $q->whereIn('user_id', $ids)->orWhereIn('author_id', $ids);
            });
        }
    ...
    /**
     * Load refunds; resolve staff via user_id / user_created_id / author_id.
    ...
    private function loadRefunds(array $filters, Collection $salesForLookup): Collection
    {
        ...
        // Parent sale map for store filter + optional staff inheritance when refund lacks user
        $saleMetaByMongo = [];
        foreach ($salesForLookup as $sale) {
            if ($sale->mongo_id) {
                $saleMetaByMongo[(string) $sale->mongo_id] = [
        ...
        foreach ($rows as $row) {
            ...
            if ($pmid && !array_key_exists((string) $pmid, $saleMetaByMongo)) {
        ...
        if ($missingPaymentMongoIds !== []) {
            $parentSales = (new MirrorRecord())->forTable('sale_payments')->newQuery()
                ...
                ->get(['mongo_id', 'branch_id', 'user_id', 'author_id']);
            foreach ($parentSales as $ps) {
                $saleMetaByMongo[(string) $ps->mongo_id] = [
                    'branch_id' => $ps->branch_id,
                    'user_id' => $ps->user_id,
                    'author_id' => $ps->author_id,
                ];
            }
        }

        return $rows->filter(function ($row) use ($filters, $saleMetaByMongo) {
            $payload = is_array($row->payload) ? $row->payload : [];

            $resolvedStaffId = $this->resolveRefundStaffId($row, $saleMetaByMongo);
            $row->_resolved_staff_id = $resolvedStaffId;

            if ($filters['staffIds'] !== []) {
                if ($resolvedStaffId === self::UNKNOWN_STAFF_ID || !in_array($resolvedStaffId, $filters['staffIds'], true)) {
                    return false;
                }
            }
            ...
            if ($filters['storeId'] !== null) {
                ...
                if (isset($row->branch_id) && $row->branch_id !== null && $row->branch_id !== '') {
                    ...
                    $pmid = $row->payment_mongo_id ?? null;
                    if ($pmid && isset($saleMetaByMongo[(string) $pmid]['branch_id'])) {
                        $bid = $saleMetaByMongo[(string) $pmid]['branch_id'];
                        $branchId = $bid !== null && $bid !== '' ? (string) $bid : null;
                    }
                }
                if ($branchId === null || $branchId !== $filters['storeId']) {
                    return false;
                }
            }
    ...
    private function resolveSaleStaffId(object $row): string
    {
        if ($row->user_id !== null && $row->user_id !== '') {
            return (string) (int) $row->user_id;
        }
        if ($row->author_id !== null && $row->author_id !== '') {
            return (string) (int) $row->author_id;
        }

        return self::UNKNOWN_STAFF_ID;
    }

    private function resolveRefundStaffId(object $row, array $saleMetaByMongo): string
    {
        if (isset($row->user_id) && $row->user_id !== null && $row->user_id !== '') {
            return (string) (int) $row->user_id;
        }
        if (isset($row->user_created_id) && $row->user_created_id !== null && $row->user_created_id !== '') {
            return (string) (int) $row->user_created_id;
        }
        if (isset($row->author_id) && $row->author_id !== null && $row->author_id !== '') {
            return (string) (int) $row->author_id;
        }

        $pmid = $row->payment_mongo_id ?? null;
        if ($pmid && isset($saleMetaByMongo[(string) $pmid])) {
            $meta = $saleMetaByMongo[(string) $pmid];
            if (!empty($meta['user_id'])) {
                return (string) (int) $meta['user_id'];
            }
            if (!empty($meta['author_id'])) {
                return (string) (int) $meta['author_id'];
            }
        }
    ...
    private function loadStaffMap(Collection $sales, Collection $refunds): Collection
    {
        $ids = [];
        foreach ($sales as $row) {
            $sid = $this->resolveSaleStaffId($row);
            if ($sid !== self::UNKNOWN_STAFF_ID && is_numeric($sid)) {
                $ids[] = (int) $sid;
            }
        }
        foreach ($refunds as $row) {
            $sid = $row->_resolved_staff_id ?? self::UNKNOWN_STAFF_ID;
            if ($sid !== self::UNKNOWN_STAFF_ID && is_numeric($sid)) {
                $ids[] = (int) $sid;
            }
        }
        ...
        if ($ids === []) {
            return User::query()->get(['id', 'name', 'email', 'role', 'is_active', 'status'])->keyBy(fn ($u) => (string) $u->id);
        }
    ...
    private function emptyStaffBucket(string $staffId, ?User $user): array
    {
        $isUnknown = $staffId === self::UNKNOWN_STAFF_ID;

        return [
            'staffId' => $staffId,
            'staffName' => $isUnknown ? 'Chưa xác định' : ($user?->name ?? 'Nhân viên #'.$staffId),
            'staffEmail' => $isUnknown ? null : ($user?->email ?? null),
            'role' => $isUnknown ? null : ($user?->role ?? null),
            'isActive' => $isUnknown ? null : ($user?->is_active === null ? true : (bool) $user->is_active),
            'grossRevenue' => 0.0,
            'discountAmount' => 0.0,
            'revenue' => 0.0,
            'refundAmount' => 0.0,
    ...
    private function aggregateByStaff(Collection $sales, Collection $refunds, Collection $staffMap): array
    {
        $buckets = [];

        foreach ($sales as $row) {
            $staffId = $this->resolveSaleStaffId($row);
            if (!isset($buckets[$staffId])) {
                $user = $staffId !== self::UNKNOWN_STAFF_ID ? $staffMap->get($staffId) : null;
                $buckets[$staffId] = $this->emptyStaffBucket($staffId, $user);
            }
            $b = &$buckets[$staffId];
        ...
        foreach ($refunds as $row) {
            $staffId = $row->_resolved_staff_id ?? self::UNKNOWN_STAFF_ID;
            if (!isset($buckets[$staffId])) {
                $user = $staffId !== self::UNKNOWN_STAFF_ID ? $staffMap->get($staffId) : null;
                $buckets[$staffId] = $this->emptyStaffBucket($staffId, $user);
            }
            $buckets[$staffId]['refundAmount'] += $this->refundAmount($row);
        }
    ...
    private function applySearch(array $staffRows, ?string $search): array
    {
        ...
        return array_filter(
            ...
            function ($row) use ($needle) {
                $name = mb_strtolower((string) ($row['staffName'] ?? ''));
                $email = mb_strtolower((string) ($row['staffEmail'] ?? ''));
                $role = mb_strtolower((string) ($row['role'] ?? ''));

                return str_contains($name, $needle) || str_contains($email, $needle) || str_contains($role, $needle);
            },
            ARRAY_FILTER_USE_BOTH
        );
    }

    private function buildSummary(array $staffRows): array
    {
        $gross = 0.0;
        $discount = 0.0;
        $revenue = 0.0;
        ...
        foreach ($staffRows as $row) {
            ...
            if ($topStaff === null || $row['netRevenue'] > $topStaff['netRevenue']) {
                $topStaff = $row;
            }
        }
        ...
        return [
            ...
            'topStaff' => $topStaff ? [
                'id' => $topStaff['staffId'],
                'name' => $topStaff['staffName'],
                'email' => $topStaff['staffEmail'],
                'netRevenue' => $topStaff['netRevenue'],
            ] : null,
        ];
    }

    private function buildRanking(array $staffRows, string $metric): array
    {
        $list = array_values($staffRows);
        usort($list, function ($a, $b) use ($metric) {
            $av = $a[$metric] ?? $a['netRevenue'] ?? 0;
            $bv = $b[$metric] ?? $b['netRevenue'] ?? 0;
            if ($av == $bv) {
                return strcmp((string) $a['staffName'], (string) $b['staffName']);
            }
    ...
    private function paginateRanking(array $ranking, array $filters): array
    {
        ...
        usort($sorted, function ($a, $b) use ($sortBy, $dir) {
            if ($sortBy === 'staffName') {
                return strcmp((string) $a['staffName'], (string) $b['staffName']) * $dir;
            }
            $av = $a[$sortBy] ?? 0;
            $bv = $b[$sortBy] ?? 0;
    ...
    private function buildTrend(
        ...
        $topIds = array_slice(array_map(fn ($r) => $r['staffId'], $ranking), 0, self::TOP_TREND_STAFF);
        if ($topIds === []) {
            return [
        ...
        foreach ($topIds as $sid) {
            $name = $sid === self::UNKNOWN_STAFF_ID
                ? 'Chưa xác định'
                : ($staffMap->get($sid)?->name ?? 'Nhân viên #'.$sid);
            $seriesData[$sid] = [
                'staffId' => $sid,
                'staffName' => $name,
                'points' => [],
        ...
        foreach ($sales as $row) {
            $staffId = $this->resolveSaleStaffId($row);
            if (!isset($seriesData[$staffId])) {
                continue;
            }
            $time = $this->eventTime($row);
            $meta = $this->bucketKey($time, $granularity);
            $key = $meta['key'];
            if (!isset($seriesData[$staffId]['_acc'][$key])) {
                $seriesData[$staffId]['_acc'][$key] = [
                    'grossRevenue' => 0.0,
                    'revenue' => 0.0,
                    'refundAmount' => 0.0,
                    'netRevenue' => 0.0,
            ...
            $acc = &$seriesData[$staffId]['_acc'][$key];
        ...
        foreach ($refunds as $row) {
            ...
            $seriesData[$staffId]['_acc'][$key]['refundAmount'] += $this->refundAmount($row);
        }
        ...
        $note = count($ranking) > self::TOP_TREND_STAFF
            ? sprintf('Biểu đồ xu hướng hiển thị top %d nhân viên theo %s.', self::TOP_TREND_STAFF, $metric)
            : null;
    ...
    private function buildBreakdowns(
        ...
        $revenueShareByStaff = array_map(fn ($r) => [
            'key' => $r['staffId'],
            'label' => $r['staffName'],
        ...
        $revenueShareByStaff = $this->normalizeBreakdown($revenueShareByStaff, 10, 'revenue');

        $byChannel = [];
        $byPayment = [];
        $byStore = [];

        $branchMap = Branch::query()->get(['id', 'name', 'code'])->keyBy(fn ($b) => (string) $b->id);
        ...
        foreach ($sales as $row) {
            ...
            $storeKey = $row->branch_id ? (string) $row->branch_id : 'unknown';
            $storeName = $storeKey === 'unknown'
                ? 'Chưa xác định'
                : ($branchMap->get($storeKey)?->name ?? 'Cửa hàng #'.$storeKey);
            if (!isset($byStore[$storeKey])) {
                $byStore[$storeKey] = ['key' => $storeKey, 'label' => $storeName, 'revenue' => 0.0, 'invoiceCount' => 0];
            }
            $byStore[$storeKey]['revenue'] += $rev;
            $byStore[$storeKey]['invoiceCount'] += 1;
        }

        return [
            'revenueShareByStaff' => $revenueShareByStaff,
            ...
            'stores' => $this->normalizeBreakdown(array_values($byStore), 10),
        ];
    }
