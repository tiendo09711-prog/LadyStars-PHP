...
try {
  ...
  console.log('[E2E] Using DB ladystars_e2e, backend 4100, frontend 5174');
  ...
  const result = await new Promise((resolve) => {
    const child = spawn('npx.cmd', ['playwright', 'test', 'tests/warehouse-branches-phase1.spec.ts', '--project=chromium', '--workers=1', '--reporter=list'], {
      cwd: e2eDir,
      env: { ...process.env },
      stdio: 'inherit',
      shell: false,
